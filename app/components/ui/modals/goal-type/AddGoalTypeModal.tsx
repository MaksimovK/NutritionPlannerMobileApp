...
import { useCreateGoalType } from '../../../../hooks/queries/goalTypes.queries'
import { IGoalType } from '../../../../types/goalTypes.types'

interface IAddGoalTypeModal {
	onClose: () => void
}

export default function AddGoalTypeModal({ onClose }: IAddGoalTypeModal) {
	const {
		control,
		handleSubmit,
		formState: { errors },
		getValues
	} = useForm<Omit<IGoalType, 'id'>>({
		mode: 'onChange'
	})

	const { mutate: createGoalType } = useCreateGoalType()

	const onSubmit = () => {
		const goalType = getValues()
		...
		>
			<View className='m-4 bg-gray-100 rounded-xl w-11/12'>
				...
				<View className='py-10 px-5'>
					<Text className='text-base text-gray-900'>Добавление типа цели</Text>
					<Controller
						control={control}
						name='name'
						rules={{
