...
import { useUpdateMealTime } from '../../../../hooks/queries/mealTimes.queries'
import { IMealTime } from '../../../../types/mealTimes.types'

interface IUpdateMealTimeModal {
	mealTime: IMealTime
	onClose: () => void
}

export default function UpdateMealTimeModal({
	mealTime,
	onClose
}: IUpdateMealTimeModal) {
	const {
		control,
		handleSubmit,
		formState: { errors }
	...
	const { mutate: updateMealTime } = useUpdateMealTime()

	const onSubmit = (data: IMealTime) => {
		updateMealTime(data)
			...
			<View className='m-4 bg-gray-100 rounded-xl w-11/12'>
				...
				<View className='py-10 px-5'>
					<Text className='text-base text-gray-900'>
						Редактирование типа приёма пищи
					</Text>
					<Controller
						control={control}
						name='name'
