...
import { useCreateMealTime } from '../../../../hooks/queries/mealTimes.queries'
import { IMealTime } from '../../../../types/mealTimes.types'

interface IAddMealTimeModal {
	onClose: () => void
}

export default function AddMealTimeModal({ onClose }: IAddMealTimeModal) {
	const {
		control,
		handleSubmit,
		formState: { errors },
		getValues
	} = useForm<Omit<IMealTime, 'id'>>({
		mode: 'onChange'
	})

	const { mutate: createMealTime } = useCreateMealTime()

	const onSubmit = () => {
		const mealTime = getValues()
