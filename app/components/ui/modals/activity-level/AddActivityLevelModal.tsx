...
import { useCreateActivityLevel } from '../../../../hooks/queries/activityLevels.queries'
import { IActivityLevel } from '../../../../types/activityLevels.types'

interface IAddActivityLevelModal {
	onClose: () => void
}

export default function AddActivityLevelModal({
	onClose
}: IAddActivityLevelModal) {
	const {
		control,
		handleSubmit,
		formState: { errors },
		getValues
	} = useForm<Omit<IActivityLevel, 'id'>>({
		mode: 'onChange'
	})

	const { mutate: createActivityLevel } = useCreateActivityLevel()

	const onSubmit = () => {
		const activityLevel = getValues()
